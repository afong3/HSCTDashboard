...
# drop = True prevents the old index from being added to the dataframe as a column
data1 = data.sort_values(by="startTimeLocal").reset_index(drop=True)
...
print(f'Cycling activities {cycling_activities}')
HSCTDistance = 29
HSCTElevGain = 1830
HSCTElevLoss = 2600
...
# makes a series so when printing use [1] index
activities_in_lap = lap_floor[lap_floor == 1].value_counts() + 1
...
# uninclusive is alright because this isn't counting the current lap
for lap in range(0, math.floor(laps_distance)):
    # [lap][lap] is funky... but pretty sure this is correct to get rid of the series
    print("Activities needed for lap " + str(lap) +
          " of HSCT Distance: " + str(lap_count_dist[lap][lap]))
    print("\n")
# Now I want the amount of days it took per lap

    # I thought this would be zero, but running is a subset of total activites
    lap_start_index = lap_floor[lap_floor == 0].index.values[0]
...
for lap in range(1, math.floor(laps_distance)+1):
    ...
    # lap - 1 is to account for starting with range(1,...)
    print("Lap " + str(lap - 1) + " started: " + str(date1))
    print("Lap " + str(lap - 1) + " finished: " + str(date2))
    # .days is needed to get ride of the time delta
    days_in_lap = (date2 - date1).days
